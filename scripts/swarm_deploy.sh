...
N="${1:-3}"
FLOOR="${REPORT_FLOOR:-8}"
START="${START_AT:-100000001}"   # skip the dense low range; must be odd
...
for i in $(seq 0 $((N - 1))); do
    ...
    sed -i "s|^#define REPORT_FLOOR       7 |#define REPORT_FLOOR       ${FLOOR} |" cunningham_w${i}.c
    sed -i "s|^#define START_AT           3ULL |#define START_AT           ${START}ULL |" cunningham_w${i}.c

    grep -E "^#define (WORKER_ID|WORKER_COUNT|REPORT_FLOOR|START_AT)" cunningham_w${i}.c | sed 's/  */ /g'
